...
def sectors(request):
    if request.method == 'POST':
        ...
        for industryClassify in industryClassifySelected:
            ...
            financialDatas[industryClassify] = queryAvgFinanceindicaForOneIndustry(startDate, endDate, industryClassify, financeIndicatorsSelected)
...
def getTopFinanceIndicators(endDate, indicatorsList):
    ...
    # if (not os.path.exists('frontEnd/templates/assets/data/sectors.json')):

        indicatorDatas = queryAvgFinanceindicaSectorsForSectors(endDate, indicatorsList)
        ...
        with open('frontEnd/templates/assets/data/sectorsIndicator.json', 'w') as fp:
