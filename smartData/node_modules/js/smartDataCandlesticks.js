    var downColor = '#607D8B';
    var upColor = '#EF5350';
    var axisLineColor = '#8392A5';

    // var upColor = '#66BB6A';
    // var downColor = '#FF7043';
    var upColorBorder = upColor;
    var downColorBorder = downColor;
    var splitNumberY= 5;
    var leftMargin = '1%';
    var rightMargin = '2%';

    //定义K线图模板
    export var option = {
         title : {
            show:true,//显示策略，默认值true,可选为：true（显示） | false（隐藏）
            text: '上证指数',//主标题文本，'\n'指定换行
            link:'',//主标题文本超链接,默认值true
            target: null,//指定窗口打开主标题超链接，支持'self' | 'blank'，不指定等同为'blank'（新窗口）
            x:'center',//水平安放位置，默认为'left'，可选为：'center' | 'left' | 'right' | {number}（x坐标，单位px）
            y: 'top',//垂直安放位置，默认为top，可选为：'top' | 'bottom' | 'center' | {number}（y坐标，单位px）
            textAlign: null,//水平对齐方式，默认根据x设置自动调整，可选为： left' | 'right' | 'center
            backgroundColor: 'rgba(0,0,0,0)',//标题背景颜色，默认'rgba(0,0,0,0)'透明
            borderWidth: 0,//标题边框线宽，单位px，默认为0（无边框）
            textStyle: {//主标题文本样式{"fontSize": 18,"fontWeight": "bolder","color": "#333"}
                fontFamily: 'Arial, Verdana, sans...',
                fontSize: 14,
                fontStyle: 'normal',
                fontWeight: 'normal',
            }
        },
        color: [
        '#2ec7c9','#b6a2de','#5ab1ef','#ffb980','#d87a80',
        '#8d98b3','#e5cf0d','#97b552','#95706d','#dc69aa',
        '#07a2a4','#9a7fd1','#588dd5','#f5994e','#c05050',
        '#59678c','#c9ab00','#7eb00a','#6f5553','#c14089'
    ],
        backgroundColor: '#fff',
        animation: false,
        legend: {
            top: 0,
            left: leftMargin,
            data: ['Dow-Jones index']
        },
        tooltip: {
            trigger: 'axis',
            axisPointer: {
                type: 'cross'
            },
            backgroundColor: 'rgba(245, 245, 245, 0.8)',
            borderWidth: 1,
            borderColor: '#ccc',
            padding: 10,
            textStyle: {
                color: '#000'
            },
            position: function (pos, params, el, elRect, size) {
                var obj = {top: 10};
                obj[['left', 'right'][+(pos[0] < size.viewSize[0] / 2)]] = 30;
                return obj;
            }
            // extraCssText: 'width: 170px'
        },
        axisPointer: {
            link: {xAxisIndex: 'all'},
            label: {
                backgroundColor: '#777'
            }
        },
        toolbox: {
            feature: {
                dataZoom: {
                    yAxisIndex: false
                },
                brush: {
                    type: ['lineX', 'clear']
                }
            }
        },
        brush: {
            xAxisIndex: 'all',
            brushLink: 'all',
            outOfBrush: {
                colorAlpha: 0.1
            }
        },
        visualMap: {
            show: false,
            seriesIndex: 5,
            dimension: 2,
            pieces: [{
                value: 1,
                color: upColor
            }, {
                value: -1,
                color: downColor
            }]
        },
        grid: [
            {
                left: leftMargin,
                right: rightMargin,
                height: '45%'
            },
            {
                left: leftMargin,
                right: rightMargin,
                top: '68%',
                height: '9%'
            },
            {
                left: leftMargin,
                right: rightMargin,
                top: '80%',
                height: '12%'
            }
        ],
        xAxis: [
            {
                type: 'category',
                data: 0,
                scale: true,
                boundaryGap: false,
                axisLine: {onZero: false ,lineStyle: { color: axisLineColor}},
                splitLine: {show: false},
                splitNumber: 20,
                min: 'dataMin',
                max: 'dataMax',
                axisPointer: {
                    z: 100
                }
            },
            {
                type: 'category',
                gridIndex: 1,
                data: 0,
                scale: true,
                boundaryGap: false,
                axisLine: {onZero: false, lineStyle: { color: axisLineColor}},
                axisTick: {show: false},
                splitLine: {show: false},
                axisLabel: {show: false},
                splitNumber: 20,
                min: 'dataMin',
                max: 'dataMax'
            }
            ,
//            {
//                type: 'category',
//                gridIndex: 2,
//                data: 0,
//                scale: true,
//                boundaryGap: false,
//                axisLine: {onZero: false, lineStyle: { color: '#8392A5' }},
//                axisTick: {show: false},
//                splitLine: {show: false},
//                axisLabel: {show: false},
//                splitNumber: 20,
//                min: 'dataMin',
//                max: 'dataMax'
//            }
            { //幅图
					type: 'category',
					gridIndex: 2,
					data: 0,
					axisLine: {onZero: false, lineStyle: { color: axisLineColor}},
					axisLabel: {
						show: false
					}
					}
        ],
        yAxis: [
            {   //主图
                splitNumber:splitNumberY,
                scale: true,
                gridIndex:0,
                splitArea: {
                    show: false
                },
                splitLine: {show: true,
                    lineStyle:{
                        type:'dotted'
                    }
                },
                axisLabel: {
                formatter: '{value}',
                inside: true //label文字朝内对齐
                },
                axisLine: {show: true, lineStyle: { color: axisLineColor}}
            },
            {   //副图1
                scale: true,
                gridIndex: 1,
                splitNumber: 2,
                axisLabel: {show: true,
                inside: true //label文字朝内对齐
                },
                axisLine: {show: true, lineStyle: { color: axisLineColor}},
                axisTick: {show: true},
                splitLine: {show: true}
            }
            ,
//            {   //副图2
//                scale: true,
//                gridIndex: 2,
//                splitNumber: 2,
//                axisLabel: {show: true},
//                axisLine: {show: true, lineStyle: { color: '#8392A5' }},
//                axisTick: {show: true},
//                splitLine: {show: true}
//            }
            { //幅图2
					z:4, gridIndex: 2,splitNumber: 4,
					axisLine: {
						onZero: false,
						lineStyle: { color: axisLineColor}
					},
					axisTick: {
						show: false
					},
					splitLine: {
						show: false,
					},
					axisLabel: { //label文字设置
						color: '#c7c7c7',
						inside: true, //label文字朝内对齐
						fontSize: 8
					}
				}

        ],
        dataZoom: [
            {
                type: 'inside',
                xAxisIndex: [0, 1,2],
                start: 0,
                end: 100
            },
            {
                type: 'slider',
                backgroundColor:"rgba(47,69,84,0.1)",  　　 //组件的背景颜色
//                dataBackground:{                         //数据阴影的样式。
//                    lineStyle:mylineStyle,               //阴影的线条样式
//                    areaStyle:myareaStyle,               //阴影的填充样式
//                },
//                fillerColor:"rgba(167,183,204,0.4)",     //选中范围的填充颜色。
                borderColor:"#BBB",                      //边框颜色
//                dataBackgroundColor: '#eceff1',
                fillerColor: 'rgba(96,125,139,0.4)',   // 填充颜色
                realtime: true,
                height: 20,
                handleColor: downColor,
                handleSize: 20,
                show: true,
                xAxisIndex: [0, 1, 2],
                //top: '92%',
                bottom:'0%',
                start: 20,
                end: 100
            }
        ],
        series: []
    };

    //解析K线数据，将其转化成{ x轴,
    //                       股价（最高、最低、开盘、收盘）,
    //                       量能}
    export function splitData(rawData) {
            var categoryData = [];
            var values = [];
            var volumes = [];
            for (var i = 0; i < rawData.length; i++) {
                categoryData.push(rawData[i].splice(0, 1)[0]);
                values.push(rawData[i]);
                volumes.push([i, rawData[i][4], rawData[i][1] > rawData[i][0] ? 1 : -1]); //交易量能
            }

            return {
                categoryData: categoryData,
                values: values,
                volumes: volumes
            };
    }

    export function splitDataByType(rawData,isPercent) {
            var categoryData = [];
            var values = [];
            var volumes = [];
            if(false == isPercent)
            {   //只有一个股票时，显示股价蜡烛图
                for (var i = 0; i < rawData.length; i++) {
                    categoryData.push(rawData[i].splice(0, 1)[0]);
                    values.push(rawData[i]);
                    volumes.push([i, rawData[i][4], rawData[i][1] > rawData[i][0] ? 1 : -1]); //交易量能
                }
            }
            else
            {   //多股同列时,显示收盘价相对于第一天开盘价的百分比折线图
                var basicValue=rawData[0][1]; //第一天的开盘价作为基准价
                for (var i = 0; i < rawData.length; i++) {
                    var time = rawData[i][0];
                    categoryData.push(rawData[i].splice(0, 1)[0]);
                    volumes.push([i, rawData[i][4], rawData[i][1] > rawData[i][0] ? 1 : -1]); //交易量能
                    values.push([String(time), (rawData[i][1]-basicValue)*100/basicValue]);   //收盘价相对于第一天开盘价的涨幅
                }
            }

            return {
                categoryData: categoryData,
                values: values,
                volumes: volumes
            };
    }

    export function calculateMA(dayCount, data) {
            var result = [];
            for (var i = 0, len = data.values.length; i < len; i++) {
                if (i < dayCount) {
                    result.push('-');
                    continue;
                }
                var sum = 0;
                for (var j = 0; j < dayCount; j++) {
                    sum += data.values[i - j][1];
                }
                result.push(+(sum / dayCount).toFixed(3));
            }
            return result;
    }

    export function setXAxisData(option, xAxisData)
    {
         option.xAxis[0].data = xAxisData;
         option.xAxis[1].data = xAxisData;
         option.xAxis[2].data = xAxisData;
    }

    export function attachStockData(option, dataName, codeData)
   {
        option.title['text'] = dataName;
        //1,添加蜡烛图k线 （当前它只显示在第一个x,y轴上）
        var candlestickItem = {
                        name: dataName,
                        type: 'candlestick',
                        data: codeData.values,
                        itemStyle: {
                                        color: upColor,
                                        color0: downColor,
                                        borderColor: upColorBorder,
                                        borderColor0: downColorBorder
                                    },
                        //////////////////////////////////////
                        markPoint: {
                                    label: {
                                                normal: {
                                                    formatter: function (param)
                                                            {
                                                                return param != null ? Math.round(param.value) : '';
                                                            }
                                                        }
                                            },
                                    data: [

                                        ]
                                    },
                        /////////////////////////////////////
                        tooltip: {
                            formatter: function (param) {
                                param = param[0];
                                return [
                                    'Date: ' + param.name + '<hr size=1 style="margin: 3px 0">',
                                    'Open: ' + param.data[0] + '<br/>',
                                    'Close: ' + param.data[1] + '<br/>',
                                    'Lowest: ' + param.data[2] + '<br/>',
                                    'Highest: ' + param.data[3] + '<br/>'
                                ].join('');
                            }
                        }
                    };
        option.series.push(candlestickItem);

        //2,设置均线
        var maItemsType=[5,10,20,30];
        for(var i=0; i<maItemsType.length; i++)
        {
            var maItem = {
                        name: maItemsType[i].toString(),
                        type: 'line',
                        data: calculateMA(maItemsType[i], codeData),
                        smooth: false,
                        symbol: "none",
                        //symbol:'emptyCircle',
                        //showAllSymbol:true,
                       // symbolSize:3,
                        lineStyle: {
                            opacity: 0.5
                        }
                    };
            option.legend.data.push(maItemsType[i].toString());
            option.series.push(maItem);
        }
        //3,设置量能副图
        var volumesItem = {
                        name: 'Volume',
                        type: 'bar',
                        xAxisIndex: 1,
                        yAxisIndex: 1,
                        data: codeData.volumes
                    };
        option.series.push(volumesItem);
   }

export function attachMacdData(option, macds)
   {
    //4,设置MACD，RPS,RSJ等指标
		var macdDIF = {
					name: 'DIF',
					type: 'line',
					symbol: "none",
					xAxisIndex: 2,
					yAxisIndex: 2,
					data: macds['dif'],
					lineStyle: {
						normal: {
							color: '#da6ee8',
							width: 1
						}
					}
				};
		var macdDEA = {
					name: 'DEA',
					type: 'line',
					symbol: "none",
					xAxisIndex: 2,
					yAxisIndex: 2,
					data: macds['dea'],
					lineStyle: {
						normal: {
							opacity: 0.8,
							color: '#39afe6',
							width: 1
						}
					}
				};
				var macdItem = {
                        name: 'MACD',
                        type: 'bar',
                        xAxisIndex: 2,
                        yAxisIndex: 2,
                        data: macds['macd'],
                        barWidth: '40%',
                        itemStyle: {
                            normal: {
                                color: function(params) {
                                    var colorList;

                                    if (params.data[1] >= 0) {
                                   // alert("111111111111111");
                                        colorList = upColor;
                                    } else {
                                    //alert("000000000000000000");
                                        colorList = downColor;
                                    }
                                    return colorList;
                                }
                            }
                        }
					};
//            var aaaaaaaa = {
//                name: 'aaaaaa',
//                type: 'bar',
//                xAxisIndex: 2,
//                yAxisIndex: 2,
//                data: macds['macd']
//            };
//            option.series.push(aaaaaaaa);
        option.series.push(macdItem);
        option.series.push(macdDIF);
        option.series.push(macdDEA);
   }


    export function attachLineData(option, dataName, xAxisDatas, data, yIndexShow)
    {   //添加简单的折线
        var newData = [];
        for (var i = 0; i < data.length; i++)
        {

            for(var j=0; j<(xAxisDatas.length-1);j++)
            {
                if(xAxisDatas[j]==data[i][0] ||
                    (xAxisDatas[j]<data[i][0] && xAxisDatas[j+1]>data[i][0]))
                    {
                       newData.push([String(xAxisDatas[j]), data[i][1]]);
                       break;
                    }
            }
        }

        var yIndex = addYAxis(option, yIndexShow, newData);
        var item = {
                        name: dataName,
                        type: 'line',
                        xAxisIndex: 0,
                        yAxisIndex: yIndex,
                        data: newData,
                        smooth: true,
                        symbol:'emptyCircle',
                        showAllSymbol:true,
                        symbolSize:3,
                        lineStyle: {
                            width:1.5,
                            type: 'solid'
                        },

                   };
        option.legend.data.push(dataName)
        option.series.push(item);
    }

    export function attachStockLineData(option, dataName, codeData, lineIndex)
    {    //为股票添加简单的折线
        var item = {
                        name: dataName,
                        type: 'line',
                        xAxisIndex: 0,
                        yAxisIndex: 0,
                        data: codeData,
                        smooth: false,
                        symbol:'circle',
                        symbolSize:10,
                        lineStyle: {
                            width:1,
                            type: 'solid'
                        },
                   };
        option.legend.data.push(dataName)
        option.series.push(item);
    }

    export function  addYAxis(option, isShow,data)
    {
        var Min = calMin(data);
        var Max = calMax(data);
       var itemShow = {
                   splitNumber: splitNumberY,
                    min: Min,
                    max: Max,
                    interval: (Max - Min) / 5,
                    splitLine: {show: true,
                        lineStyle:{
                            type:'dotted'
                        }
                    },
                    scale: true,
                    gridIndex:0,
                    position: "right",
                    "axisLine":{       //y轴
                      "show":true
                    },
                    "axisTick":{       //y轴刻度线
                      "show":true
                    }
                   };

       var itemHide = {
                    splitNumber: splitNumberY,
                    min: Min,
                    max: Max,
                    interval: (Max - Min) / 5,
                    splitLine: {show: true,
                        lineStyle:{
                            type:'dotted'
                        }
                    },
                    scale: true,
                    gridIndex:0,
                    position: "right",
                    "axisLine":{       //y轴
                      "show":false
                    },
                    "axisTick":{       //y轴刻度线
                      "show":false
                    },
                    axisLabel : {
                        formatter: function(){
                              return "";
                        }
                   }
            };
       if(isShow)
       {
       option.yAxis.push(itemShow);
       }
       else
       {
       option.yAxis.push(itemHide);
       }

       return (option.yAxis.length-1);
    }


    export function resetSeriesData(option)
    {
            option.series.splice(0, option.series.length);
            option.yAxis.splice(3, option.yAxis.length-3);
    }

    export function resetMacroData(option)
    {
        if(option.series.length>6)
        {
            option.series.splice(6, option.series.length-5);
            option.yAxis.splice(2, option.yAxis.length-2);
            option.legend.data.splice(5, option.legend.data.length-5);
        }
    }

    export function makePoints(option, baseData, macroData)
    {
        for (var i = 0; i < macroData.length; i++)
        {
            for(var j=0; j<(baseData.categoryData.length-1);j++)
            {
                if(baseData.categoryData[j]==macroData[i][0] ||
                    (baseData.categoryData[j]<macroData[i][0] && baseData.categoryData[j+1]>macroData[i][0]))
                    {
                       var item={
                                  name: i,
                                   coord: [String(baseData.categoryData[j]), baseData.values[j][0]],
                                   value: macroData[i][1],
                                   itemStyle: {
                                       color: 'rgb(41,60,85)'
                                              }
                              };
                               option.series[0].markPoint.data.push(item);
                    }
            }
        }
    }

    //计算最大值
 export function calMax(arr) {
    let max = 0;

    for(var i=0; i<(arr.length);i++)
    {
          if (max < arr[i][1]) {
            max = arr[i][1];
            }
    }
    return max*1.1;
  }

  //计算最小值
 export function calMin(arr) {
    let min = 0;
    for(var i=0; i<(arr.length);i++)
    {
          if (min > arr[i][1]) {
            min = arr[i][1];
            }
    }
    return min;
  }
